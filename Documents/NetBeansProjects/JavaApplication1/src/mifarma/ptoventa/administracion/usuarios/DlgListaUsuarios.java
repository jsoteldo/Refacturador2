...
public class DlgListaUsuarios extends JDialog {
            ...
            if (FarmaVariables.vEconoFar_Matriz)
                ...
                if (tieneRegistroSeleccionado(this.tblListausuarios)) {
                    if (JConfirmDialog.rptaConfirmDialog(this,
                        ...
                        //LTAVARA 2017.08.21 
                        if(inactivarUsuarioMaeTrab(tblListausuarios.getValueAt(tblListausuarios.getSelectedRow(), 12).toString().trim()).equals("N")){
                                FarmaUtility.showMessage(this,
                                                         "No puede Activar un Usuario Inactivo por el área de RRHH.",
                                                         txtApePaterno);
                                return;
                            }
                        cargarUsuarioSeleccionado();
    ...
    private boolean busca_cajeros_caja() {
        ...
        String resultado = "";
        try {
            resultado = DBUsuarios.buscaUsuSinCajaAsignada(sec_usuario);
     ...
     * Obtiene el estado de la caja
     */
    private String inactivarUsuarioMaeTrab(String num_documento) {
        String estado = "";
        try {
            estado = DBUsuarios.validarInactivarUsuarioMaeTrab(num_documento);
            log.debug("inactivarUsuarioMaeTrab :  " + estado);
        } catch (SQLException e) {
            log.error("", e);
        }
