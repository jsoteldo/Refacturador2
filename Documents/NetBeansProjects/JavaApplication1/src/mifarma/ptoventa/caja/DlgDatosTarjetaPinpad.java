...
public class DlgDatosTarjetaPinpad extends JDialog {
    ...
    private JButtonLabel lblObsTarjeta = new JButtonLabel();
    ...
    private void jbInit() throws Exception {
        ...
        jLabelWhite1.setText("Datos de tarjeta de pago");
        ...
        txtNroTarjeta.setBounds(new Rectangle(115, 20, 135, 20));
        ...
        pnlInfo.add(lblObsTarjeta, null);
        ...
        lblObsTarjeta.setText("<HTML><CENTER>Primeros\n06 Digitos</HTML></CENTER>");
        lblObsTarjeta.setBounds(new Rectangle(15, 30, 100, 25));
        lblObsTarjeta.setForeground(new Color(155, 79, 9));
        lblObsTarjeta.setFocusable(false);
        lblObsTarjeta.setFont(new Font("SansSerif", 1, 9));
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        if (FarmaVariables.vTipCambio == 0) {
            ...
            if(VariablesFidelizacion.vCodFormaPagoCampanasR.equalsIgnoreCase(ConstantsFidelizacion.COD_FPAGO_TARJ_OH)){
                this.setTitle("Pago con descuento exclusivo para Tarjeta Oh!");
                jLabelWhite1.setText("Datos de la Tarjeta Oh!");
            }
    ...
    private void txtnrotarj_keyTyped(KeyEvent e) { 
        FarmaUtility.admitirDigitos(txtNroTarjeta,e);
    }
    ...
    private void txtnrotarj_keyReleased(KeyEvent e) {
        int NroDigitos = txtNroTarjeta.getText().trim().length();
        if (e.getKeyCode() != KeyEvent.VK_ENTER && NroDigitos>6){
                FarmaUtility.showMessage(this, "Solo debe ingresar los 6 primeros digitos de la tarjeta.", null);
                String nros = txtNroTarjeta.getText().trim().substring(0, 6);
                txtNroTarjeta.setText(nros);
                FarmaUtility.moveFocus(txtNroTarjeta);
        }
    }
    ...
    private void txtnrotarj_keyPressed(KeyEvent e) {
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER){
            ...
            if(NroDigitos==6){
                String nros = txtNroTarjeta.getText().trim().substring(0, 6);
                String textoFormateado = String.format("%-16s", nros).replace(' ','*');        
                txtNroTarjeta.setText(textoFormateado);
            }else if(NroDigitos<6){
                FarmaUtility.showMessage(this, "Ingrese los 06 primeros digitos de la tarjeta para continuar.", null);
                FarmaUtility.moveFocus(txtNroTarjeta);
            }
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if ((strIndLectorBanda.equals("S") && intCountEnter == 2) ||
                ...
                if (buscarInfotarjeta()) {
                    log.info("strCodFormaPago: "+strCodFormaPago);
                    if (hablitarTipoTarjeta()) {
                        ...
                        log.info("Error en hablitarTipoTarjeta()");
    ...
    private boolean buscarInfotarjeta() {
        ...
        log.info(" ==> Cant. Registros lista: "+lista.size());
        if (lista.size() > 0) {
            ...
            if (vNroTarjeta.length() > 0 && (vNroTarjeta.length() == 15 || vNroTarjeta.length() == 16)) {
                ...
                nroTarjeta = "";
                flag = false;
                log.info("Error en buscarInfotarjeta()");
        ...
        if (vNroTarjeta.length() > 0) {
            ...
            if (!UtilityPtoVenta.validarNumero_01(vNroTarjeta)) {
                flag = false;
            ...
            //INI LMEZA 04/01/2019
                                   ...
                                   strCodFormaPago.equals(ConstantsPtoVenta.FORPAG_MC_POS)) {
                            if (!(vNroTarjeta.length() == 14 || vNroTarjeta.length() == 16)) {
                                flag = false;
                            }
                        }
    ...
    public void conciliacionPinpad(DlgInterfacePinpad dlgInterfacePinpad) {
        String vSalida = "";
