...
public class DlgListaRemito extends JDialog {
    ...
    private void cambiarCodigoRemito() {
        ...
        //INICIO VALIDACIÓN LMEZA 30.04.18
        String codRemiNEW = "";
        codRemiNEW = FarmaUtility.ShowInput(this, "Ingrese Codigo Remito");
        if(codRemiNEW.trim().length() == 0){
        do {
           codRemiNEW = FarmaUtility.ShowInput(this, "Ingrese Codigo de Nuevo Remito");
           }while(codRemiNEW.trim().length() == 0);
        }
        
        String precinto = "";
        if (UtilityCajaElectronica.getIndImpreRemito_Matricial()){
            precinto = FarmaUtility.ShowInput(this, "Ingrese Codigo Precinto");
            if(precinto.trim().length() == 0){
            do {
               precinto = FarmaUtility.ShowInput(this, "Ingrese Codigo de Nuevo Precinto");
               }while(precinto.trim().length() == 0);
            }
        }
        //FIN VALIDACIÓN LMEZA 30.04.18
        try {
