...
public class DlgGenerarTransfDelivery extends JDialog {
    ...
    private boolean procesarTransferencia() {
        ...
        try {
            ...
            FarmaUtility.liberarTransaccion();
          /// jvara 18-08-2017  log.error("", sql);  
            if (sql.getErrorCode() == 20666) {
                ...
                if(sql!=null && sql.getErrorCode()==20081){   /// jvara 18-08-2017 inicio
                         log.debug("codigo de error de sql  " + sql.getErrorCode());  
                         FarmaUtility.showMessage(this, "Ha ocurrido un error al grabar la transferencia.\n" +
                                 sql.getMessage(), null); 
                     }
                else {
                    log.error("", sql);                        /// jvara 18-08-2017 fin
                    FarmaUtility.showMessage(this, "Ha ocurrido un error al grabar la transferencia.\n" +
                            sql.getMessage(), null);
                }
            }
