...
public class DlgKardex extends JDialog {
    ...
    private void txtBuscar_keyPressed(KeyEvent e) {
        ...
        //LMEZA 24/01/19 Las teclas ESC, F2, F3, F4, F8, F9 y F12 funcionan sin tener seleccionado ningún producto.
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE){
            cerrarVentana();
        } else if (!tieneRegistroSeleccionado(tblListaProductos)&&
            (e.getKeyCode() == KeyEvent.VK_F2 || e.getKeyCode() == KeyEvent.VK_F3 || e.getKeyCode() == KeyEvent.VK_F4 
             || e.getKeyCode() == KeyEvent.VK_F8 || e.getKeyCode() == KeyEvent.VK_F9 || e.getKeyCode() == KeyEvent.VK_F12)){
            FarmaUtility.showMessage(this, "Debe seleccionar un producto activo para realizar esta operación.", txtBuscar);
    ...
    private boolean verificarProductoInctivo() {
        if(!tieneRegistroSeleccionado(tblListaProductos)){ //LMEZA 25/01/18
            return true;
        } else {
    ...
    private void chkKeyPressed(KeyEvent e) {
        ...
        if(prodInactivo && e.getKeyCode() != KeyEvent.VK_ESCAPE){
            //muestraMsj("El producto seleccionado esta marcado como inactivo, selecione otro!");
            return;
        }
        ...
        if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F2(e)) {
            ...
            //if (!FarmaVariables.dlgLogin.verificaRol(FarmaConstants.ROL_SUPERVISOR_VENTAS)) { //AOVIEDO 22/01/2018
