...
public class DlgConteoRecepMercaderia_02 extends JDialog {
        ...
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (txtBuscar.getText().trim().length() > 15) {
                ...
                if (tmpCodBarra.trim().length() == 0) {
                    ...
                    if((validaProductoAlmacen(tmpCodBarra.trim())))
    ...
    /*** INICIO CCASTILLO 14/02/2017 ***/
    public boolean validaProductoAlmacen(String tmpCodBarra){
        log.info("validaProductoAlmacen (tmpCodBarra) :" + tmpCodBarra);
        boolean flag=true;
        try{
            DBRecepCiega.validaProductoAlmacen(VariablesRecepCiega.vNumIngreso,tmpCodBarra);   
        }catch (SQLException sql) {
           if (sql.getErrorCode() == 20010 || sql.getErrorCode() == 20011) {
               txtBuscar.setText("");
               flag=false;
               FarmaUtility.showMessage(this, sql.getMessage().substring(11, sql.getMessage().indexOf("ORA-06512")),txtBuscar);
           }else { // aquí no agregar el flag=false porque es otro tipo de error y sera controlado por la función ingresaProducto
                log.error("", sql);
                /*FarmaUtility.showMessage(this, "Ocurrió un error al agregar el producto.\n" +
