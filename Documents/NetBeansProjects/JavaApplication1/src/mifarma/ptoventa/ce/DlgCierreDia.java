...
public class DlgCierreDia extends JDialog {
    ...
    private void jbInit() throws Exception {
        ...
        /*** INICIO CCASTILLO 21/08/2017 ***/
        pnlMensajeDollar.setBounds(new Rectangle(10, 200, 130, 25));
        ...
        lblMensajeDollar.setBounds(new Rectangle(5, 5, 120, 15));
        ...
        jContentPane.add(pnlMensajeDollar, null);
        /*** FIN CCASTILLO 21/08/2017 ***/
    ...
    private boolean validaFechaCierreDia() {
        ...
        String indPermiteCierre;
        try {
            indPermiteCierre = DBCajaElectronica.PermitirCierreCaja(diaVenta);
        } catch (SQLException e) {
            e.printStackTrace();
            indPermiteCierre = "N";
        }
        
        if(indPermiteCierre.equalsIgnoreCase("S")){
            int cont = 1;
            ...
            FarmaUtility.showMessage(this, "El local no tiene permitido realizar el cierre de dia\n" +
                                    "Consulte con mesa de ayuda", txtFecha);
            return false;
        }
    }
    ...
    private boolean validaVBCotComp() {
        String ind = "";
        try {
            ind = DBCajaElectronica.validaVBCotComp(VariablesCajaElectronica.vFechaCierreDia,
                                                    VariablesCajaElectronica.vCodCuadratura);
        } catch (SQLException sql) {
            log.error("", sql);
            FarmaUtility.showMessage(this, "Error al contabilizar el VB de la Cuadratura de Cotización Competencia.\n" +
                    sql, tblCuadraturas);
            ind = "";
        }
        if (ind.equalsIgnoreCase(FarmaConstants.INDICADOR_S))
    ...
    private void cambiaVBCierreDia() {
        ...
        if(validaRolContador() && !validaVBCotComp()){
                FarmaUtility.showMessage(this, 
                "No puede dar VoBo del día porque tiene Detalles de Cuadratura Cotización Competencia con Validación negativo.\n", tblCuadraturas);
                 return;
    ...
    private void imprimeCierreDia() {
      ...
      */
            if (cargarProductosVirtuales()) {
                ...
                vPrint.printBold(FarmaPRNUtility.llenarBlancos(15) + "Tipo Producto      Cantidad  Total "+ConstantesUtil.simboloSoles+" ", true);
    ...
    /*private void guardaInformacionComprobantes()
  {
    ...
    private boolean validaFechaLimiteUso() {
        return false;
    }
}
