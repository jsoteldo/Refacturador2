...
public class DlgFormaPagoEntrega extends JDialog {
    ...
    private String COND_AGREGAR = "AGREGAR"; //AOVIEDO 26/05/2017
    private String COND_BORRAR = "BORRAR"; //AOVIEDO 26/05/2017
    /* ********************************************************************** */
    /*                        CONSTRUCTORES                                   */
    /* ********************************************************************** */
    ...
    private void jbInit() throws Exception {
        ...
        lblSaldoT.setText("TOTAL A ENTREGAR :  "+ConstantesUtil.simboloSoles+" ");
        ...
        tblFormasPago.addKeyListener(new KeyAdapter() {
            ...
            //INI AOVIEDO 22/05/17
            public void keyReleased(KeyEvent e){
                tblFormasPago_keyReleased(e);
            }
            //FIN AOVIEDO 22/05/17
        });
        ...
        txtLote.setDocument(new FarmaLengthText(5));
    ...
    private void tblFormasPago_keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ENTER) {
            e.consume();
            validaFormaPagoSeleccionada(true); // AOVIEDO 22/05/17
        }
    ...
    //INI AOVIEDO 22/05/17
    private void tblFormasPago_keyReleased(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_DOWN || e.getKeyCode() == KeyEvent.VK_UP) {
            e.consume();
            validaFormaPagoSeleccionada(false);
        }
    ...
    private void btnFormaPago_actionPerformed(ActionEvent e) {
        ...
        validaFormaPagoSeleccionada(false); // AOVIEDO 22/05/17
    }
    ...
    private void btnAdicionar_actionPerformed(ActionEvent e) {
        ...
        if(validaOPER(COND_AGREGAR).equals("S")){
            cargaLogin();
        } 
    ...
    private void chkKeyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            ...
            //INI AOVIEDO 26/05/2017
            if(validaOPER(COND_BORRAR).equals("S")){
                cargaLogin();
            } 
            //FIN AOVIEDO 26/05/2017
    ...
    private void validaFormaPagoSeleccionada(boolean indEnter) { // AOVIEDO 22/05/17
        ...
        if (codFormaPago.equalsIgnoreCase(ConstantsCaja.FORMA_PAGO_EFECTIVO_SOLES)) {
            ...
            txtLote.setText("");
            txtCantidad.setText("");
            if(indEnter) FarmaUtility.moveFocus(txtMontoPagado); // AOVIEDO 22/05/17
            ...
            btnAdicionar.setEnabled(true);
            // JMIRANDA 19.07.2010
            if(indEnter) FarmaUtility.moveFocus(txtLote); // AOVIEDO 22/05/17
            //FarmaUtility.moveFocus(txtCantidad);
        } else if (indTarjeta.equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
            log.debug("FORMA DE PAGO - TARJETA");
            FarmaLoadCVL.setSelectedValueInComboBox(cmbMoneda, FarmaConstants.HASHTABLE_MONEDA,
            ...
            txtMontoPagado.setEnabled(true);
            btnAdicionar.setEnabled(true);
            if(indEnter) FarmaUtility.moveFocus(txtCantidad); // AOVIEDO 22/05/17
        }
        //JMIRANDA 10.03.2010
                    ...
                    if (com.gs.mifarma.componentes.JConfirmDialog.rptaConfirmDialog(this,
                        ...
                        if (VariablesCajaElectronica.vIndSobre.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                            ...
                            try {
                                ...
                                if (VariablesPtoVenta.vIndTico.equals("S") &&
                                    VariablesPtoVenta.vIndConFarma.equals("S")) {
                                    // JVARA Antes de eliminar el sobre de tico, verifica si es tico  
    ...
    private void imprimeSobresDeclarados_0X(ArrayList listaS, String pSecMovCaja) {
        String pIndProsegur = FarmaConstants.INDICADOR_N;
        boolean indImp = false;
        String indicadorImpresion = ""; //jvara 28-08-2017
        try {
            //pIndProsegur = DBCajaElectronica.getIndProsegur().trim();
            ...
            if (pIndProsegur.trim().equalsIgnoreCase(FarmaConstants.INDICADOR_S)) {
                ...
                if (!vIndImpre.equals("N")) {
                    for (int f = 0; f < listaS.size(); f++) {
                        indicadorImpresion="original"; //jvara 28-08-2017
                        pCodSobre = FarmaUtility.getValueFieldArrayList(listaS, f, 0);
                        VariablesPtoVenta.vImprimeOriginalCopia=indicadorImpresion; //jvara 28-08-2017
                        String html = DBCajaElectronica.getHtmlSobreDeclarados(pSecMovCaja, pCodSobre);
                        String[] arrayHtml = html.split("@");   //jvara 28-08-2017
                        html=arrayHtml[0]+arrayHtml[1]+arrayHtml[2];   //jvara 28-08-2017  
    ...
    //INI AOVIEDO 26/05/2017
    private String validaOPER(String pCondicion){
        String indValidaOPER = "N";
        String codFormaPago = "";
        
        if(pCondicion.equals(COND_AGREGAR)){
            if (tblFormasPago.getRowCount() > 0){
                int fila = tblFormasPago.getSelectedRow();
                codFormaPago = ((String)tblFormasPago.getValueAt(fila, 0)).trim();
                log.debug("codFormaPago : " + codFormaPago);
                
                try{
                    indValidaOPER = DBCajaElectronica.obtieneIndicadorValidaOPER(codFormaPago);
                } catch (SQLException sql) {
                    log.error("", sql);
                }
            }
        }else{
            codFormaPago = tblDetallePago.getValueAt(tblDetallePago.getSelectedRow(), COL_COD_FPAGO).toString();
            log.debug("codFormaPago : " + codFormaPago);
