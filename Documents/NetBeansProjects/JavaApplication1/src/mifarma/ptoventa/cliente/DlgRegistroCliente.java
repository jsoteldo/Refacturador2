...
public class DlgRegistroCliente extends JDialog {
    ...
    private boolean pIndAtencionMedica;
    ...
    private void chkKeyPressed(KeyEvent e) {
        if (mifarma.ptoventa.reference.UtilityPtoVenta.verificaVK_F11(e)) {
            ...
            if (resValidacion.length() == 0) {
                ...
                if(pIndAtencionMedica){
                    cerrarVentana(true);
                }else if (insertarCliente())
                    cerrarVentana(true);
                else
    ...
    private boolean insertarCliente() {
        ...
        Map<String, String> parametros = datosPersonales();
        try {
    ...
    private void this_windowOpened(WindowEvent e) {
        ...
        //ERIOS 23.08.2016 Se muestran etiquetas de Aten. Med.
        if(pIndPaciente){
            this.setTitle("Datos de Paciente");
            lblDniCliente.setText("DNI Paciente: *");
        }else if(pIndAtencionMedica){
            this.setTitle("Datos de Paciente");
            lblDniCliente.setText("Doc. Paciente:");
            lblApeMaterno.setText("Apellido Materno:");
            lblFechaNac.setText("Fecha Nacimiento:");
            lblGenero.setText("Genero:");
            jPanel1.setBounds(new Rectangle(15, 40, 465, 150));
            lblEsc.setBounds(new Rectangle(380, 200, 95, 20));
            lblF11.setBounds(new Rectangle(265, 200, 105, 20));
            this.setSize(new Dimension(502, 255));
        }
    ...
    private String validarFormulario() { //validar campos obligatorios vacios
        ...
        if (txtApeMaterno.getText().length() <= 0 && !pIndAtencionMedica)
            respVacios = respVacios + "+ Apellido Materno\n";
        if (txtNombre.getText().length() <= 0)
            respVacios = respVacios + "+ Nombre\n";
        if (txtFechaNac.getText().length() <= 0 && !pIndAtencionMedica)
        ...
        if (txtFechaNac.getText().length() > 0 && !UtilityRecetario.validarDDMMYYYY(txtFechaNac.getText()))
    ...
    public void setPIndAtencionMedica(boolean pIndAtencionMedica) {
        this.pIndAtencionMedica = pIndAtencionMedica;
    }
    
    public Map<String, String> recuperarPaciente() {        
        Map<String, String> parametros = datosPersonales();
        return parametros;
    }
}
