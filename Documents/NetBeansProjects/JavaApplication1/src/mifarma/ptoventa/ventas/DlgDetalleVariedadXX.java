...
public class DlgDetalleVariedadXX extends JDialog {

    /* ************************************************************************ */
    /*                          DECLARACION PROPIEDADES                         */
    /* ************************************************************************ */
    private static final Logger log = LoggerFactory.getLogger(DlgDetalleVariedadXX.class);
    ...
    public DlgDetalleVariedadXX() {
        this(null, "", false, "");
    }

    public DlgDetalleVariedadXX(Frame parent, String title, boolean modal, String indTipoPack) {
     ...
     */
        ...
        if(pAceptar){
            // ingreso productos de promo nuevo no seleccionado            
            VariablesVentas.vArrayList_Prom_Aux_XX = new ArrayList();
            VariablesVentas.vArrayList_Prod_Prom_Aux_XX = new ArrayList();
            
            VariablesVentas.vArrayList_Prod_Prom_Aux_XX = (ArrayList)VariablesVentas.vArrayList_Prod_Promociones_temporal.clone();
            VariablesVentas.vArrayList_Prom_Aux_XX = (ArrayList)VariablesVentas.vArrayList_Promociones_temporal.clone();
    ...
    private void listaPaquetes() {
        try {
            // cargarDatos();
            myArray.clear();
            DBVentas.obtieneListadoPaquetes02(myArray, VariablesVentas.vCodProm.trim());
        ...
        try {
            ...
            if (indTipoPack.equals(ConstantsVentas.IND_PACK_VARIEDAD)) {
                ...
                if(listaLimites.size() > 0 && listaLimites != null && !listaLimites.isEmpty()){
                    ...
                    //porcPaquete2 = ((ArrayList)listaLimites.get(0)).get(3).toString().trim(); //AOVIEDO 16/08/2017
                }
                ...
                */
                ...
                for (int c = 0; c < tableModelListaPaquete1.data.size(); c++) {
                    ...
                    if (!cantComodin.equals(" ")) {                        
                        porcComodin = listComodin.get(8).toString().trim();
                        precioComodin = listComodin.get(4).toString().trim();
    ...
    private void separarProductosArray() {
        ...
        int stock = 0;
        String codProd = "";
        ArrayList listCloned = (ArrayList)myArray.clone();
        for (int c = 0; c < listCloned.size(); c++) {
            listTemp = new ArrayList();
            stock = Integer.parseInt(FarmaUtility.getValueFieldArrayList(myArray, c, 9).trim());
            codProd = FarmaUtility.getValueFieldArrayList(myArray, c, 0);
            if (stock > 1) {
                listTemp.add((ArrayList)myArray.get(c));
                ((ArrayList)listTemp.get(0)).set(9, "1");
                removeItemArray(myArray, codProd, 0);
                for (int i = 0; i < stock; i++) {
                    myArray.add(((ArrayList)listTemp.get(0)).clone());
                }
            }
        }
    }
    ...
    private void ponerPorcetajesDsctoMyArray() {        
        ...
        //double porcPaquete2 = Double.parseDouble(((ArrayList)listaLimites.get(0)).get(3).toString()); //AOVIEDO 16/08/2017
        ...
        log.debug("ANTES PONER PORC: " + myArray);
        
        for (int c = 0; c < (cantMaxPaq2 * cantPacks); c++) {
            int i = c + cantPacks;
            
            list = (ArrayList)myArray.get(i);
            precio = Double.parseDouble(list.get(5).toString());
            porcPaquete2 = Double.parseDouble(list.get(24).toString()); //AOVIEDO 16/08/2017
            //INI AOVIEDO 26/04/17
            ...
            list.set(5,  " " + precioDescontado);//precio descontado
...
*/
        ...
        log.debug("DESPUES PONER PORC: " + myArray);
    }
    ...
    private void limpiarProductosVariedad() {
            ...
            for (int i = 0; i < tableModelListaPaquete1.data.size(); i++) {
                ...
                if (codProd.equals(codProdPq)) {
                    if (cantPq.equals(" ")) {
                        ...
                        ((ArrayList)myArray.get(c)).set(9, cantPq);
                    }
                    break;
                }
            }
        ...
        log.info("ARRAY PROCESADO: " + myArray);
    ...
    private boolean validaStockActual() {
        ...
        for (int i = 0; i < auxNuevo.size(); i++) {
            ...
            if (tipoProducto.trim().equals(ConstantsVentas.TIPO_PROD_FINAL)) {
                int cantidad = Integer.parseInt(lblCantidad.getText().trim());
                if (!UtilityVentas.existsStockComp(this, codProd, cantidad, cantidad)) {
                ...
                // MODIFICAR PARA VALIDAR STOCK
                // DUBILLUZ 2017.05.02
                if ((Integer.parseInt(unidad)) <= Integer.parseInt(disponible))
                    valor = true;
                else
                    return false;
            }
    ...
    private void AceptarProductosComprometidos() {
        ...
        try {
            ...
            for (int i = 0; i < myArray.size(); i++) {
                ...
                //dubilluz 2017.05.02
                VariablesVentas.vCant_Ingresada = FarmaUtility.getValueFieldArrayList(myArray, i, 9);
         ...
         */
                ...
                myArray2.add(VariablesVentas.vCod_Prod);//0
                myArray2.add(VariablesVentas.vDesc_Prod);//1
                myArray2.add(VariablesVentas.vUnid_Vta);//2
                myArray2.add(VariablesVentas.vVal_Prec_Lista);//3
                myArray2.add(VariablesVentas.vCant_Ingresada); //ingresa en Double //4
                myArray2.add("" + FarmaUtility.getDecimalNumber(VariablesVentas.vPorc_Dcto_1)); //proc_Dcto_1; //5
                myArray2.add(VariablesVentas.vVal_Prec_Vta); // 6
                myArray2.add(FarmaUtility.formatNumber(totaltemp, 2) + "");//7
                myArray2.add(VariablesVentas.vVal_Bono); //val_Bono //8
                myArray2.add(VariablesVentas.vNom_Lab); //9
                myArray2.add(VariablesVentas.vVal_Frac);//10
    ...
    */
    ...
    private ArrayList agruparProdPaquete(ArrayList array) {
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            if (aux1.size() < 26) { 
                ...
                for (int j = i + 1; j < array.size(); j++) {
                    aux2 = (ArrayList)(array.get(j));
                    if (aux2.size() < 26) { //JCHAVEZ 20102009 DECIA <21
                        if ((((String)(aux1.get(0))).trim()).equalsIgnoreCase((((String)(aux2.get(0))).trim()))) {
    ...
    private int getCantIngresadaPaquete(String opcion) {
        int valor = 0;
        int cantIngresadaProd = 0;
        int cantIngresadaParaPack = 0;
        if (opcion.equals("1")) {
            for (int c = 0; c < tableModelListaPaquete1.data.size(); c++) {
                ...
                if (!fila.get(0).toString().equals(FarmaUtility.getValueFieldArrayList(tableModelListaPaquete1.data, tblpaquete1.getSelectedRow(), 0))) {
                    cantIngresadaProd = Integer.parseInt((fila.get(5).toString().trim().equals("") ? "0" : fila.get(5).toString()));
                    cantIngresadaParaPack = cantIngresadaProd / Integer.parseInt(fila.get(12).toString());
                    valor = valor + cantIngresadaParaPack;
                }                
            }
    ...
    private int getCantIngresadaPaqueteTotal(String opcion) {
        ...
        if (opcion.equals("1")) {
            for (int c = 0; c < tableModelListaPaquete1.data.size(); c++) {
                ArrayList fila = (ArrayList)tableModelListaPaquete1.data.get(c);
                cantIngresadaProd = Integer.parseInt((fila.get(5).toString().trim().equals("") ? "0" : fila.get(5).toString()));
                valor = valor + (cantIngresadaProd / Integer.parseInt(fila.get(12).toString()));
            }
    ...
    private boolean isPackCompleto () {
        ...
        if (cantAtendida == cantSolicitada) {
            flag = true;
            lblMsgCompletado.setText("EL PACK YA ESTA COMPLETO");
        } else {
