...
public class DlgListaProductosReceta extends JDialog {
    ...
    public void iniciaProceso(boolean pInicializar) {
        ...
        if (pInicializar) {
            ...
            for (int i = 0; i < VariablesCentroMedico.vArrayList_ResumenReceta.size(); i++)
                VariablesCentroMedico.vArrayList_PedidoReceta.add((ArrayList)VariablesCentroMedico.vArrayList_ResumenReceta.get(i));
        }
        log.debug("VariablesCentroMedico.vArrayList_PedidoReceta : " + VariablesCentroMedico.vArrayList_PedidoReceta.size());
        ArrayList myArray = new ArrayList();
        for (int i = 0; i < VariablesCentroMedico.vArrayList_PedidoReceta.size(); i++)
            myArray.add((ArrayList)VariablesCentroMedico.vArrayList_PedidoReceta.get(i));
            ...
            try {
                ...
                if (myJTable.getSelectedRow() >= 0) {
                    ...
                    if (productoBuscar.length() > 6 &&
                        ...
                        VariablesCentroMedico.vCodigoBarra = productoBuscar;
    ...
    private void muestraInfoProd() {
        ...
        VariablesCentroMedico.vVal_Frac = valFracProd;
        VariablesCentroMedico.vInd_Prod_Habil_Vta = indProdHabilVta;
        myJTable.repaint();
    }
    ...
    private void muestraIngresoCantidad() {
        if (myJTable.getRowCount() == 0)
            return;
        VariablesCentroMedico.vCod_Prod = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(), 1))).trim();
        VariablesCentroMedico.vDesc_Prod = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(), 2))).trim();
        VariablesCentroMedico.vNom_Lab = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(), 4))).trim();
        VariablesCentroMedico.vPorc_Igv_Prod = ((String)(myJTable.getValueAt(myJTable.getSelectedRow(), 11))).trim();
    ...
    private void colocaTotalesPedido() {
        ...
        totalItems = VariablesCentroMedico.vArrayList_PedidoReceta.size();
    ...
    private void calculaTotalVentaPedido() {
        ...
        for (int i = 0; i < VariablesCentroMedico.vArrayList_PedidoReceta.size(); i++)
            totalVenta +=
                    FarmaUtility.getDecimalNumber(((String)((ArrayList)VariablesCentroMedico.vArrayList_PedidoReceta.get(i)).get(7)).trim());
    }
    ...
    private void deseleccionaProducto() {
        ...
        String codigoTmp = "";
        for (int i = 0; i < VariablesCentroMedico.vArrayList_PedidoReceta.size(); i++) {
            codigoTmp = (String)((ArrayList)VariablesCentroMedico.vArrayList_PedidoReceta.get(i)).get(0);
            if (VariablesCentroMedico.vCod_Prod.equalsIgnoreCase(codigoTmp)) {
                cantidad = (String)((ArrayList)VariablesCentroMedico.vArrayList_PedidoReceta.get(i)).get(4);
                break;
            }
        }
        Boolean valorTmp = (Boolean)(myJTable.getValueAt(myJTable.getSelectedRow(), 0));
    ...
    private void operaProductoSeleccionadoEnArrayList(Boolean valor) {
        ArrayList myArray = new ArrayList();
        myArray.add(VariablesCentroMedico.vCod_Prod);
        myArray.add(VariablesCentroMedico.vDesc_Prod);
        myArray.add(VariablesCentroMedico.vUnid_Vta);
        myArray.add(VariablesCentroMedico.vVal_Prec_Lista);
        myArray.add(VariablesCentroMedico.vCant_Ingresada);
        myArray.add(VariablesCentroMedico.vPorc_Dcto_1);
        myArray.add(FarmaUtility.formatNumber(FarmaUtility.getDecimalNumber(VariablesCentroMedico.vVal_Prec_Vta), 3));
        myArray.add(FarmaUtility.formatNumber(VariablesCentroMedico.vTotalPrecVtaProd, 2));
        myArray.add(VariablesCentroMedico.vVal_Bono);
        myArray.add(VariablesCentroMedico.vNom_Lab);
        myArray.add(VariablesCentroMedico.vVal_Frac);
        myArray.add(VariablesCentroMedico.vPorc_Igv_Prod);
        String valIgv =
        ...
        myArray.add(VariablesCentroMedico.vVal_Igv_Prod);
        ...
        try {
            DBVentas.obtieneInfoProductoVta(infoProd, VariablesCentroMedico.vCod_Prod);
        ...
        FarmaUtility.operaListaProd(VariablesCentroMedico.vArrayList_PedidoReceta, myArray, valor, 0);
        //log.debug("size : " + VariablesCentroMedico.vArrayList_PedidoReceta.size());
        //log.debug("array : " + VariablesCentroMedico.vArrayList_PedidoReceta);
    }
    ...
    private void aceptaOperacion() {
        ...
        for (int i = 0; i < VariablesCentroMedico.vArrayList_PedidoReceta.size(); i++)
            VariablesCentroMedico.vArrayList_ResumenReceta.add((ArrayList)VariablesCentroMedico.vArrayList_PedidoReceta.get(i));
        cerrarVentana(true);
    }
    ...
    private void actualizaListaProductosAlternativos() {
        ...
        if (esProductoFarma()) {
            muestraProductosAlternativos();
            FarmaUtility.ponerCheckJTable(tblListaProductosAlternativos, 1, VariablesCentroMedico.vArrayList_PedidoReceta,
                                          0);
        }
    }
    ...
    private void pintaCheckOtroJTable(JTable pActualJTable, Boolean pValor) {
        ...
        if (pActualJTable.getName().equalsIgnoreCase(ConstantsVentas.NAME_TABLA_PRODUCTOS_RECETA)) {
            FarmaUtility.setearCheckInRow(tblListaProductosAlternativos, pValor, true, true, VariablesCentroMedico.vCod_Prod,
                                          1);
            tblListaProductosAlternativos.repaint();
        } else if (pActualJTable.getName().equalsIgnoreCase(ConstantsVentas.NAME_TABLA_PRODUCTOS_ALTERNATIVOS_RECETA)) {
            FarmaUtility.setearCheckInRow(tblProductos, pValor, true, true, VariablesCentroMedico.vCod_Prod, 1);
